...
export default function Pruebas() {
  ...
  async function abrirAdjunto(adj) {
    try {
      if (adj.pdf_origen === 'drive') {
        openDriveFile(adj.pdf_drive_id);
      } else {
            ...
            <div style={{ background: 'var(--teal-50)', border: '1.5px dashed var(--teal-300)', borderRadius: 10, padding: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                <button type="button" onClick={() => cameraRef.current.click()}
                  ...
                  {procesando ? 'Procesando...' : 'Hacer foto'}
                </button>
              ...
              {adjuntos.length > 0 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {adjuntos.map((file, idx) => (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'white', border: '1px solid var(--teal-100)', borderRadius: 8, padding: '8px 12px' }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--teal-500)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        {file.type.startsWith('image/')
                          ? <><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></>
                          : <><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></>
                        }
                      </svg>
                      <span style={{ fontSize: 11, color: 'var(--teal-700)', flex: 1, wordBreak: 'break-all' }}>{file.name}</span>
                      <button type="button" onClick={() => setAdjuntos(prev => prev.filter((_, i) => i !== idx))}
                        style={{ background: 'none', border: 'none', color: 'var(--slate-400)', cursor: 'pointer', fontSize: 18, lineHeight: 1, flexShrink: 0 }}>×</button>
                    </div>
                  ))}
                </div>
              )}
