  ...
  useEffect(() => {
    ...
    async function cargar() {
      try {
        ...
        const qCitas = query(collection(db, 'citas'), where('uid', '==', user.uid), where('fecha', '>=', fechaHoy), orderBy('fecha', 'asc'), limit(6));
